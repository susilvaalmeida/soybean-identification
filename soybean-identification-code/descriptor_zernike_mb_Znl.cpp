...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
  ...
  int i ;
  std::complex<double> sum = 0.0 ;    /* Accumulator for complex moments */
  std::complex<double> Vnl = 0.0 ;    /* Inner sum in Zernike calculations */
  ...
  if (nrhs != 5) {
    mexErrMsgTxt("MB_Znl(N, L, X, Y, P), Zernike moment generating function.  The moment of degree n and angular dependence l for the pixels defined by coordinate vectors X and Y and intensity vector P.  X, Y, and P must have the same length.") ;
  } else if (nlhs != 1) {
  ...
  preal[0] = std::real(sum) ;
  pimag[0] = std::imag(sum) ;
