function [acvector sensitivity]=classify_script_new(tClassifier,featureType)
...
if ( strcmp(upper(tClassifier),'SVM')~=1 && strcmp(upper(tClassifier),'KNN')~= 1 && strcmp(upper(tClassifier),'NV')~= 1)
    fprintf(1,'Classifier type invalid!\n');
    return;
end

outName = '';
if(strcmp(upper(tClassifier),'SVM')==1)
    outName = 'svm_';
elseif(strcmp(upper(tClassifier),'KNN')==1)
    %neigh = '1';
    %neigh = '3';
   % neigh = '5';
    %neigh = '7';
    ...
    outName = ['knn_' neigh '_'];
elseif(strcmp(upper(tClassifier),'NV')==1)
    outName = 'nv_';
end
        ...
        for j=1:length(tq)
            ...
            for k=1:length(nts)
                for l=1:length(metrics)
                    ...
                    load([directoryFeatures  'folders5' '.mat'],'damages','leaves','pool');
                    ...
                    elseif(strcmp(upper(tClassifier), 'KNN')==1)
                        parameters = str2num(neigh);
                    elseif(strcmp(upper(tClassifier), 'NV')==1)
                        parameters = 'NULL';
                    end
                    
                    [accuracy sensitivity,Cc,Cb,Bc,Bb] = svmknnnv_validation(tClassifier,matrix,damages,parameters,fOut);
                    ...
                    save ([directoryResults outName filename],'Cc','Cb','Bc','Bb');
                end
            end
            fprintf(fOutTex,'\\\\ \n');
        end
