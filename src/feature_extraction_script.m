function feature_extraction_script(featureType)
        ...
        for j=1:length(tq)
            for k=1:length(nts)
                for l=1:length(metrics)
                    ...
                    it=it+1;
                    fprintf(1,'%03d - ',it);fprintf(1,'%s %5.3f %5.3f %2d - ',[sm{:}],t0(i),tq(j),nts(k));fprintf(1,'%d ',fix(clock));;fprintf(1,'\n');
                    feature_extraction(featureType,{t0(i),tq(j),nts(k),sm});
                end
            end
        end
    end
elseif strcmp(featureType,'fr')
    ...
    for i=1:length(nSize)
        ...
        feature_extraction(featureType,{nSize(i)});
    end
    ...
    for i=1:length(base)
        ...
        for j=1:length(nSize1)
            for k=1:length(pSize2)
                ...
                feature_extraction(featureType,{[bs{:}],nSize1(j),pSize2(k)});
            end
        end
    end
...
%    nDegree = [7,8,9,10,11,12];
    ...
    for i=1:length(nDegree)
        ...
        feature_extraction(featureType,{nDegree(i)});
    end
end