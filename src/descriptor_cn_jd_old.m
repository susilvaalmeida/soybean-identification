function vprob = descriptor_cn_jd(img,t,sm)
...
%    t = t0:tinc:tq;
    ...
    p=0;
    if sum(sm == 'e') p=p+1; pos(1) = p; end
    if sum(sm == 'h') p=p+1; pos(2) = p; end
    if sum(sm == 'p') p=p+1; pos(3) = p; end
    ...
    for i=1:nt
        ...
        if sum(sm == 'e') vprob(ssm*(i-1)+pos(1)) = E; end
        if sum(sm == 'h') vprob(ssm*(i-1)+pos(2)) = H; end
        if sum(sm == 'p') vprob(ssm*(i-1)+pos(3)) = P; end
    end
end